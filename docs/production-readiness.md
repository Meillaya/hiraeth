...
`mix gate` is not a substitute for the deep lane: the fast blocking gate deliberately omits sobelow/hex.audit (network-dependent, covered by the CI `static` job and the deep `ci` lane), dialyzer, coverage, assets, provenance, ingestion drills, sidecar pytest, scripts tests, and release image builds. A release owner must run the deep lane before any production-ready claim.
...
4. Frontend logic verification: the LiveView and route logic test suites under `test/hiraeth_web/live/` and `test/hiraeth_web/`, which verify public browser routes and UI behavior without a browser-level QA lane.
...
| --- | --- | --- |
| Frontend verification | LiveView and route logic test suites under `test/hiraeth_web/` | `test/hiraeth_web/live/*` |
...
- Ingestion drills, sidecar pytest, and CI use the consolidated script paths above.
