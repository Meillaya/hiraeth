defmodule HiraethWeb.CoreComponents do
  @moduledoc """
  ...
  with project-specific components written directly in HEEx. Useful references:
  ...
  use Gettext, backend: HiraethWeb.Gettext
  ...
  defp error(assigns) do
    ~H"""
    <p class="mt-1.5 flex items-center gap-2 text-sm text-red-700 dark:text-red-300">
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", "pb-4"]}>
      <div>
        ...
        <p :if={@subtitle != []} class="text-sm text-stone-600 dark:text-stone-400">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(HiraethWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(HiraethWeb.Gettext, "errors", msg, opts)
    end
  end
