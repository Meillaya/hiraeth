...
| `importer.ex` | idempotent trusted Ash writer (1091 lines). `seed!/1` whole corpus; `seed_provider!/3` single provider inside `Ash.transact` with 30-min timeout and stale pruning |
...
- Per-record import order: publisher → imprint → work (`sync_work_metadata!` guarded by `source_safe_work_update?`) → edition (`find_or_create_edition!` + `sync_edition_metadata!`) → `ensure_identifier!` → `ensure_contributions!` → `ensure_series_memberships!` → `ensure_cover!` → `ensure_source_record!` + `ensure_source_ledger_entry!` → `prune_stale_source_records!`.
