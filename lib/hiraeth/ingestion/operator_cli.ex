defmodule Hiraeth.Ingestion.OperatorCLI do
  ...
  defp check_sidecar_health do
    case sidecar_client().health() do
      ...
      _error ->
        {:error,
         "Scrapling sidecar is not running. For local devenv use, start the devenv-managed scrapling-sidecar process declared in devenv.nix on loopback, or debug it from a devenv shell with: cd sidecar && uv run --extra dev uvicorn app.main:app --host 127.0.0.1 --port 8000. Set SCRAPLING_SIDECAR_URL=http://127.0.0.1:8000 for Phoenix/operator tasks. Production runs on Railway, where Phoenix reaches the private sidecar over the Railway private network; release images build from the root Dockerfile and sidecar/Dockerfile."}
    end
  end
