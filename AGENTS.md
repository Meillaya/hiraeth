...
- `mix test.fast` excludes `slow`, `full_catalog`, `integration`, `performance`, `browser`, `public_catalog_full`. `mix test.full` is the full suite minus the opt-in `:nightly` lane; bare `mix test` also skips `:nightly` by default. `:nightly` is the opt-in lane: excluded by default from every lane, re-enabled with `--include nightly`, and owned by the deep-lane nightly coverage job (86.1 floor).
